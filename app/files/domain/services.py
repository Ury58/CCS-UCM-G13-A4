...
# # from fastapi import UploadFile, HTTPException
# # from app.files.persistence.minio import MinioClient
# # from app.auth.dependency_injection.token import get_redis_client
# # from fastapi.responses import FileResponse

# # class FileService:
# #     def __init__(self, minio_client: MinioClient, redis_client):
# #         self.minio_client = minio_client
# #         self.redis_client = redis_client
...
# # def get_file_service(minio_client: MinioClient = Depends(get_minio_client), redis_client = Depends(get_redis_client)) -> FileService:
# #     return FileService(minio_client, redis_client)
...
#     def get_file_url(self, user: str, file_id: int) -> str:
#         for name, id in file_names.items():
#             if id == file_id:
#                 url = self.minio_client.get_file_url(name)
#                 return url
...
def get_file_service(minio_client: MinioClient = Depends(get_minio_client), redis_client: RedisClient = Depends(get_redis_client)) -> FileService:
    return FileService(minio_client, redis_client)
